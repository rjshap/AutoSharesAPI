...
The trading APIs are designed to perform operations typically performed by trader's widgets: orders, trading, account values, buying powers, positions, transaction details, watchlists and price alerts, and so forth. These APIs can be called from anywhere simply using your app's API key and a user's credentials. 
