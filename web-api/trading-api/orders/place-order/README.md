...
Feel free to examine our code sample that demonstrates how to place a new order with the help of a Python script.

The following covers the syntax for this API request in PYTHON in detail.

import requests

class EtnaAPIRequest:

	baseURL = "https://URL GOES HERE"
	AppKey = "App Key for AUTH"
...
curl -X POST --header 'Content-Type: application/json' --header 'Accept: application/json' --header 'Authorization: theToken' --header 'Et-App-Key: EtAppKey' -d '{ \ 
   ...
   "TrailingStopAmountType" : "Absolute",  \ 
   "TrailingStopAmount" : 10, \ 
 }' 'https://URL GOES HERE /api/v1.0/accounts/6303/orders'
